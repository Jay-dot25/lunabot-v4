#!/usr/bin/env python3
"""Static Phase F gate: semantic terrain mapping (5-class)."""
...
check("root launch-f resolves its real path", "readlink -f" in wrapper and "SCRIPT_PATH" in wrapper)
check("root launch-f execs scripts/launch-f.sh", "scripts/launch-f.sh" in wrapper)
check("Phase F does not invoke an earlier launcher", "launch-e" not in noncomment and "launch-d" not in noncomment and "launch-c" not in noncomment and "launch-b" not in noncomment and "launch-a" not in noncomment)
check("Phase F has its own evidence directory", "phase-f-launch-f" in launch and "phase-e-launch-e" not in noncomment)
...
check("Phase F cleans mapper process", "MAPPER_PID" in launch and 'stop_group "$MAPPER_PID"' in launch)
...
check("Phase F preserves one SLAM system", "ros2 launch slam_toolbox online_async_launch.py" in launch)

check("mapper is a ROS node", "class SemanticTerrainMapper(Node)" in mapper)
check("mapper consumes the Phase E mask", "Image, self.mask_topic" in mapper and "_mask_callback" in mapper)
check("mapper consumes depth", "Image, self.depth_topic" in mapper and "_depth_callback" in mapper)
check("mapper defines 5-class input", "BEDROCK = 0" in mapper and "REGOLITH = 1" in mapper and "ROCK = 2" in mapper)
check("mapper defines 5-class grid encoding", "BEDROCK_GRID" in mapper and "REGOLITH_GRID" in mapper and "ROCK_GRID" in mapper and "CRATER_GRID" in mapper and "SHADOW_GRID" in mapper)
check("mapper preserves 5 classes not collapsed", "INPUT_TO_GRID" in mapper and "PRIORITY" in mapper)
check("mapper gives hazardous precedence", "ROCK_GRID" in mapper and "CRATER_GRID" in mapper)
check("mapper publishes OccupancyGrid", "OccupancyGrid" in mapper and "self.map_pub.publish" in mapper)
check("mapper publishes auditable status", "SEMANTIC_MAP_PASS" in mapper and "self.status_pub" in mapper)
check("mapper map/status are late-join safe", mapper.count("TRANSIENT_LOCAL") >= 2)
check("mapper does not publish motion commands", "/cmd_vel" not in mapper)
check("mapper status includes 5-class counts", "bedrock=" in mapper.lower() and "regolith=" in mapper.lower() and "rock=" in mapper.lower())
...
check("Phase F validates real semantic messages", 'topic_ok "topic /lunabot/terrain/semantic_map"' in launch and 'topic_ok "topic /lunabot/terrain/semantic_map/status"' in launch)
check("Phase F validates semantic content", "SEMANTIC_MAP_PASS" in launch and "semantic map content: PASS" in launch)
check("Phase F records semantic evidence", "semantic_map_sample.txt" in launch and "semantic_map_status.txt" in launch and "semantic_mapping.log" in launch)
check("Phase F retains segmentation content gate", "SEGMENTATION_PASS" in launch)
check("Phase F retains A* goal gate", "wait_for_goal" in launch)
check("Phase F retains map motion evidence", "map_before_navigation.txt" in launch and "map_after_navigation.txt" in launch)
...
check("Phase F RViz keeps segmentation overlay", "Terrain Segmentation Overlay" in rviz and "Value: /lunabot/terrain/overlay" in rviz)
check("Phase F RViz shows semantic map", "Semantic Terrain Map" in rviz and "Value: /lunabot/terrain/semantic_map" in rviz)
...
print("=" * 64)
print("LUNABOT V4 PHASE F STATIC VALIDATION (5-class)")
print("=" * 64)
