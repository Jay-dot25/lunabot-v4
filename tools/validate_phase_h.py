#!/usr/bin/env python3
"""Static Phase H gate: terrain-aware weighted path planning (5-class graded)."""
...
check("Phase H does not invoke an earlier launcher", "launch-g" not in noncomment and "launch-f" not in noncomment and "launch-e" not in noncomment)
check("Phase H has its own evidence directory", "phase-h-launch-h" in launch)
...
check("Phase H cleans terrain planner process", "TERRAIN_PLANNER_PID" in launch and 'stop_group "$TERRAIN_PLANNER_PID"' in launch)
...
check("Phase H preserves one SLAM system", "ros2 launch slam_toolbox online_async_launch.py" in launch)
...
check("planner uses existing localization", "lookup_transform" in planner)
check("planner uses weighted A* with graded costs", "heapq" in planner and "cost_weight" in planner)
...
check("planner plan/status are late-join safe", planner.count("TRANSIENT_LOCAL") >= 1)
check("planner does not publish motion commands", "/cmd_vel" not in planner)
check("planner status includes avg cost and graded legend", "avg_cost" in planner and "graded" in planner.lower())
...
check("Phase H validates real plan messages", 'topic_ok "topic /lunabot/terrain/plan"' in launch and 'topic_ok "topic /lunabot/terrain/planner/status"' in launch)
check("Phase H validates plan content", "TERRAIN_PLAN_PASS" in launch and "terrain-aware plan content: PASS" in launch)
check("Phase H records planner evidence", "terrain_plan.txt" in launch and "terrain_planner_status.txt" in launch)
check("Phase H retains cost content gate", "COST_MAP_PASS" in launch)
check("Phase H retains semantic content gate", "SEMANTIC_MAP_PASS" in launch)
check("Phase H cost_weight 2.5", "cost_weight:=2.5" in launch)

check("Phase H RViz keeps map fixed frame", "Fixed Frame: map" in rviz)
check("Phase H RViz keeps cost map", "Terrain Cost Map" in rviz and "Value: /lunabot/terrain/cost_map" in rviz)
check("Phase H RViz shows terrain-aware plan", "Terrain-aware Plan" in rviz and "Value: /lunabot/terrain/plan" in rviz)

passed = sum(ok for _, ok, _ in checks)
print("=" * 64)
print("LUNABOT V4 PHASE H STATIC VALIDATION (5-class graded)")
print("=" * 64)
