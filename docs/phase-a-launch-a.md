...
| World physics and presentation set | Terrain split into high-detail visual + low-detail collision mesh (fast DART physics). Lunar gravity `0 0 -1.62` is preserved and the scene uses low lunar ambient light, a near-black background, and disabled shadows. The static `lunar_habitat` has a 5 m cylindrical pressure-vessel base, overlapping physical dome, and projecting cylindrical airlock; each major component has both visual and collision geometry. The horizon catch-plane, equipment module, visible rock, and forward physical obstacle remain available as physical landmarks. |
| Launch script | `scripts/launch-a.sh` rewritten: 10 staged startup steps with live `OK/FAIL`, real runtime validation (topic + TF checks, not assumptions), `HEADLESS`/`DEMO`/`EVIDENCE` modes, trap-based clean shutdown with orphan guard, log capture to `evidence/phase-a-launch-a/`. Gazebo is explicitly started with `-r` in both server-only and GUI modes; without it, Fortress can load the world paused while merely advertising `/clock` and odometry topics. Runtime checks require non-empty message samples and cannot pass merely because a shell pipeline ended successfully. |
