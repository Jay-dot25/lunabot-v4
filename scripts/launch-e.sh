#!/bin/bash
# ============================================================
#  LunaBot V4 - Phase E  (launch-e)
#  AUTONOMOUS NAVIGATION
# ============================================================
#  Independent launch. It starts the Phase A world and rover directly,
#  then inserts the Phase E ROS control and odometry-monitoring layer.
...
# that group. This prevents a Phase E relaunch from inheriting old children.
stop_group() {
  local pid="${1:-}"
  [ -n "$pid" ] || return 0
...
shutdown() {
  ...
  say ""
  say "------------------------------------------------------------"
  say " Shutting down LunaBot Phase E safely..."
  say "------------------------------------------------------------"
...
# ------------------------------------------------------------
# [2/15] Check ROS 2 / Gazebo environment
# ------------------------------------------------------------
echo "[2/15] Checking ROS 2 / Gazebo environment........"
...
log "[2/15] environment OK ($IGN/$MSGNS, $BRIDGE_PKG, slam_toolbox)"

# ------------------------------------------------------------
# [3/15] Clean state
# ------------------------------------------------------------
echo "[3/15] Checking for stale Phase A/E processes......."
...
sleep 1
echo "      clean state: OK"
log "[3/15] clean state OK"

# ------------------------------------------------------------
# [4/15] Start Gazebo - same validated Phase A world
# ------------------------------------------------------------
echo "[4/15] Starting Gazebo lunar world.................."
...
# sensors, DiffDrive odometry and the Phase E demo can actually advance.
...
log "[4/15] gazebo OK and unpaused (pid $GAZEBO_PID)"

# ------------------------------------------------------------
# [5/15] Spawn LunaBot V4
# ------------------------------------------------------------
echo "[5/15] Spawning LunaBot V4.........................."
...
log "[5/15] spawn OK (z=$SPAWN_Z)"

# ------------------------------------------------------------
# [6/15] ROS 2 <-> Gazebo bridge - same sensor contract
# ------------------------------------------------------------
echo "[6/15] Starting ROS 2 <-> Gazebo bridge............."
...
log "[6/15] bridge OK (pid $BRIDGE_PID)"

# ------------------------------------------------------------
# [7/15] Phase E RGB-D terrain segmentation
# ------------------------------------------------------------
echo "[7/15] Starting terrain segmentation................"
...
log "[7/15] segmentation OK (pid $PERCEPTION_PID)"

# ------------------------------------------------------------
# [8/15] Static TF - unchanged Phase A sensor frames
# ------------------------------------------------------------
echo "[8/15] Starting static TF (sensor frames)............"
TF_SPECS=(
...
log "[8/15] static TF OK"

# ------------------------------------------------------------
# [9/15] Phase E control node
# ------------------------------------------------------------
echo "[9/15] Starting Phase E control layer................"
...
log "[9/15] control OK (pid $CONTROL_PID)"

# ------------------------------------------------------------
# [10/15] Phase E odometry monitor
# ------------------------------------------------------------
echo "[10/15] Starting odometry monitor...................."
...
log "[10/15] odometry monitor OK (pid $ODOM_PID)"

# ------------------------------------------------------------
# [11/15] Phase E SLAM and localization
# ------------------------------------------------------------
echo "[11/15] Starting slam_toolbox mapping................"
...
log "[11/15] slam_toolbox OK (pid $SLAM_PID)"

# ------------------------------------------------------------
# [12/15] Phase E A* autonomous navigation
# ------------------------------------------------------------
echo "[12/15] Starting A* autonomous navigation..........."
...
log "[12/15] A* navigation OK (pid $NAV_PID, auto_goal=$AUTO_GOAL)"

# ------------------------------------------------------------
# [13/15] RViz2
# ------------------------------------------------------------
if [ "$HEADLESS" = "1" ]; then
  echo "[13/15] RViz2 (skipped - HEADLESS).................. OK"
  log "[13/15] rviz skipped (headless)"
else
  echo "[13/15] Starting RViz2.............................."
  setsid rviz2 -d "$RVIZ_CONFIG" -f map >> "$EVIDENCE_DIR/gazebo.log" 2>&1 &
  RVIZ_PID=$!
  sleep 2
  if kill -0 "$RVIZ_PID" 2>/dev/null; then
    echo "      RViz2 running (PID $RVIZ_PID), fixed frame: map"
    log "[13/15] rviz OK (pid $RVIZ_PID)"
  else
    echo "      WARNING: RViz2 exited at startup (continuing without it)."
    log "[13/15] rviz FAILED to start"
    OVERALL="FAIL"
  fi
fi

# ------------------------------------------------------------
# [14/15] Runtime validation
# ------------------------------------------------------------
echo "[14/15] Runtime validation.........................."
wait_for_goal() {
...
if [ "$EVIDENCE" = "1" ]; then
  echo ""
  echo "Recording Phase E runtime evidence -> $EVIDENCE_DIR"
...
echo "  evidence/phase-e-launch-e/map_sample.txt"
echo "  evidence/phase-e-launch-e/phase_e_map.yaml/.pgm"
echo "  evidence/phase-e-launch-e/odometry_samples.csv"
echo "  evidence/phase-e-launch-e/odometry_report.txt"
echo "  evidence/phase-e-launch-e/demo_drive_result.txt"
echo "  evidence/phase-e-launch-e/diag_drive.csv"
echo ""
echo "------------------------------------------------------------"
echo "VALIDATION"
echo "------------------------------------------------------------"
...
echo ""
echo "============================================================"
log "[15/15] status printed (overall $OVERALL)"
...
if [ "$DEMO" = "1" ]; then
  ...
  echo ""
  echo "============================================================"
  printf "PHASE E RUN COMPLETE - overall result: %s\n" "$OVERALL"
  echo "============================================================"
  shutdown "$EXIT_CODE"
else
  echo ""
  echo "============================================================"
  echo " Phase E autonomous navigation is running."
  echo ""
  echo " A* auto goal: $AUTO_GOAL"
  echo " RViz can publish a replacement goal on /goal_pose."
  echo " Press Ctrl+C to stop Phase E safely."
  echo "============================================================"
