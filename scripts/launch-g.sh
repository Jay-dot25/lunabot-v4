#!/bin/bash
# ============================================================
#  LunaBot V4 - Phase G  (launch-g)
#  AUTONOMOUS NAVIGATION
# ============================================================
#  Independent launch. It starts the Phase A world and rover directly,
#  then inserts the Phase G ROS control and odometry-monitoring layer.
...
RVIZ_CONFIG="$REPO_DIR/rviz/phase_g.rviz"
EVIDENCE_DIR="$REPO_DIR/evidence/phase-g-launch-g"
...
# that group. This prevents a Phase G relaunch from inheriting old children.
stop_group() {
  local pid="${1:-}"
  [ -n "$pid" ] || return 0
...
save_final_map() {
  ...
  timeout 30 ros2 run nav2_map_server map_saver_cli -f "$EVIDENCE_DIR/phase_g_map" \
    --ros-args -p save_map_timeout:=10.0 2>/dev/null \
    >> "$EVIDENCE_DIR/map_saver.log" 2>&1 || true
  if [ -s "$EVIDENCE_DIR/phase_g_map.yaml" ] && [ -s "$EVIDENCE_DIR/phase_g_map.pgm" ]; then
...
shutdown() {
  ...
  say ""
  say "------------------------------------------------------------"
  say " Shutting down LunaBot Phase G safely..."
  say "------------------------------------------------------------"
  ...
  say "Launch G environment cleanly closed."
  say "Log: $LOG_FILE"
  say "============================================================"
  exit "$rc"
}
...
# ------------------------------------------------------------
# [1/17] Check project files
# ------------------------------------------------------------
echo "[1/17] Checking Phase A baseline + Phase G files....."
missing=0
for f in "$WORLD_PATH" "$MODEL_PATH" "$WASD_PATH" "$CONTROL_PATH" "$ODOM_PATH" "$SLAM_CONFIG" "$NAV_PATH" \
         "$RVIZ_CONFIG" "$PERCEPTION_PATH" "$MAPPER_PATH" "$COST_PATH" "$WORLD_DIR/meshes/lunar_terrain.obj" \
...
if [ "$missing" -ne 0 ]; then
  echo "      ERROR: required Phase A/G files are missing."
  exit 1
fi
echo "      world, rover, terrain, teleop, controller, monitor, planner, perception, mapper, cost mapper, rviz: OK"
log "[1/17] files OK"

# ------------------------------------------------------------
# [2/17] Check ROS 2 / Gazebo environment
# ------------------------------------------------------------
echo "[2/17] Checking ROS 2 / Gazebo environment........"
...
log "[2/17] environment OK ($IGN/$MSGNS, $BRIDGE_PKG, slam_toolbox)"
...
# only processes belonging to this Phase G command/node contract.
...
sleep 1
echo "      clean state: OK"
log "[3/17] clean state OK"

# ------------------------------------------------------------
# [4/17] Start Gazebo - same validated Phase A world
# ------------------------------------------------------------
echo "[4/17] Starting Gazebo lunar world.................."
...
# sensors, DiffDrive odometry and the Phase G demo can actually advance.
...
log "[4/17] gazebo OK and unpaused (pid $GAZEBO_PID)"

# ------------------------------------------------------------
# [5/17] Spawn LunaBot V4
# ------------------------------------------------------------
echo "[5/17] Spawning LunaBot V4.........................."
...
log "[5/17] spawn OK (z=$SPAWN_Z)"

# ------------------------------------------------------------
# [6/17] ROS 2 <-> Gazebo bridge - same sensor contract
# ------------------------------------------------------------
echo "[6/17] Starting ROS 2 <-> Gazebo bridge............."
...
log "[6/17] bridge OK (pid $BRIDGE_PID)"

# ------------------------------------------------------------
# [7/17] Phase G RGB-D terrain segmentation
# ------------------------------------------------------------
echo "[7/17] Starting terrain segmentation................"
...
log "[7/17] segmentation OK (pid $PERCEPTION_PID)"

# ------------------------------------------------------------
# [8/17] Phase G semantic terrain mapper
# ------------------------------------------------------------
echo "[8/17] Starting semantic terrain mapping..........."
...
log "[8/17] semantic mapper OK (pid $MAPPER_PID)"

# ------------------------------------------------------------
# [10/17] Static TF - unchanged Phase A sensor frames
# ------------------------------------------------------------
# ------------------------------------------------------------
# [9/17] Phase G terrain cost-map generator
# ------------------------------------------------------------
echo "[9/17] Starting terrain cost-map generator........."
...
log "[9/17] cost mapper OK (pid $COST_PID)"

# ------------------------------------------------------------
# [10/17] Static TF - unchanged Phase A sensor frames
# ------------------------------------------------------------
echo "[10/17] Starting static TF (sensor frames)............"
TF_SPECS=(
...
log "[10/17] static TF OK"

# ------------------------------------------------------------
# [11/17] Phase G control node
# ------------------------------------------------------------
echo "[11/17] Starting Phase G control layer................"
...
log "[11/17] control OK (pid $CONTROL_PID)"

# ------------------------------------------------------------
# [12/17] Phase G odometry monitor
# ------------------------------------------------------------
echo "[12/17] Starting odometry monitor...................."
...
log "[12/17] odometry monitor OK (pid $ODOM_PID)"

# ------------------------------------------------------------
# [13/17] Phase G SLAM and localization
# ------------------------------------------------------------
echo "[13/17] Starting slam_toolbox mapping................"
...
log "[13/17] slam_toolbox OK (pid $SLAM_PID)"

# ------------------------------------------------------------
# [14/17] Phase G A* autonomous navigation
# ------------------------------------------------------------
echo "[14/17] Starting A* autonomous navigation..........."
...
log "[14/17] A* navigation OK (pid $NAV_PID, auto_goal=$AUTO_GOAL)"

# ------------------------------------------------------------
# [15/17] RViz2
# ------------------------------------------------------------
if [ "$HEADLESS" = "1" ]; then
  echo "[15/17] RViz2 (skipped - HEADLESS).................. OK"
  log "[15/17] rviz skipped (headless)"
else
  echo "[15/17] Starting RViz2.............................."
  setsid rviz2 -d "$RVIZ_CONFIG" -f map >> "$EVIDENCE_DIR/gazebo.log" 2>&1 &
  RVIZ_PID=$!
  sleep 2
  if kill -0 "$RVIZ_PID" 2>/dev/null; then
    echo "      RViz2 running (PID $RVIZ_PID), fixed frame: map"
    log "[15/17] rviz OK (pid $RVIZ_PID)"
  else
    echo "      WARNING: RViz2 exited at startup (continuing without it)."
    log "[15/17] rviz FAILED to start"
    OVERALL="FAIL"
  fi
fi

# ------------------------------------------------------------
# [16/17] Runtime validation
# ------------------------------------------------------------
echo "[16/17] Runtime validation.........................."
wait_for_goal() {
...
if [ "$EVIDENCE" = "1" ]; then
  echo ""
  echo "Recording Phase G runtime evidence -> $EVIDENCE_DIR"
...
echo "  evidence/phase-g-launch-g/map_sample.txt"
echo "  evidence/phase-g-launch-g/phase_g_map.yaml/.pgm"
echo "  evidence/phase-g-launch-g/odometry_samples.csv"
echo "  evidence/phase-g-launch-g/odometry_report.txt"
echo "  evidence/phase-g-launch-g/demo_drive_result.txt"
echo "  evidence/phase-g-launch-g/diag_drive.csv"
echo ""
echo "------------------------------------------------------------"
echo "VALIDATION"
echo "------------------------------------------------------------"
...
echo ""
echo "============================================================"
log "[17/17] status printed (overall $OVERALL)"
...
if [ "$DEMO" = "1" ]; then
  ...
  echo ""
  echo "============================================================"
  printf "PHASE G RUN COMPLETE - overall result: %s\n" "$OVERALL"
  echo "============================================================"
  shutdown "$EXIT_CODE"
else
  echo ""
  echo "============================================================"
  echo " Phase G autonomous navigation is running."
  echo ""
  echo " A* auto goal: $AUTO_GOAL"
  echo " RViz can publish a replacement goal on /goal_pose."
  echo " Press Ctrl+C to stop Phase G safely."
  echo "============================================================"
