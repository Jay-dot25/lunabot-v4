#!/bin/bash
# ============================================================
#  LunaBot V4 - Phase J  (launch-j)
#  AUTONOMOUS NAVIGATION
# ============================================================
#  Independent launch. It starts the Phase A world and rover directly,
#  then inserts the Phase J ROS control and odometry-monitoring layer.
...
RVIZ_CONFIG="$REPO_DIR/rviz/phase_j.rviz"
EVIDENCE_DIR="$REPO_DIR/evidence/phase-j-launch-j"
...
# that group. This prevents a Phase J relaunch from inheriting old children.
stop_group() {
  local pid="${1:-}"
  [ -n "$pid" ] || return 0
...
save_final_map() {
  ...
  if [ "$MAP_SAVER_AVAILABLE" != "1" ]; then
    ...
    log "validation FAIL: Phase J requires nav2_map_server for final map evidence"
    OVERALL="FAIL"
    return 1
  fi
  [ -n "$SLAM_PID" ] || return 0
  ...
  timeout 30 ros2 run nav2_map_server map_saver_cli -f "$EVIDENCE_DIR/phase_j_map" \
    --ros-args -p save_map_timeout:=10.0 2>/dev/null \
    >> "$EVIDENCE_DIR/map_saver.log" 2>&1 || true
  if [ -s "$EVIDENCE_DIR/phase_j_map.yaml" ] && [ -s "$EVIDENCE_DIR/phase_j_map.pgm" ]; then
...
shutdown() {
  ...
  say ""
  say "------------------------------------------------------------"
  say " Shutting down LunaBot Phase J safely..."
  say "------------------------------------------------------------"
  ...
  say "Launch J environment cleanly closed."
  say "Log: $LOG_FILE"
  say "============================================================"
  exit "$rc"
}
...
# ------------------------------------------------------------
# [1/20] Check project files
# ------------------------------------------------------------
echo "[1/20] Checking Phase A baseline + Phase J files....."
missing=0
for f in "$WORLD_PATH" "$MODEL_PATH" "$WASD_PATH" "$CONTROL_PATH" "$ODOM_PATH" "$SLAM_CONFIG" "$NAV_PATH" \
         "$RVIZ_CONFIG" "$PERCEPTION_PATH" "$MAPPER_PATH" "$COST_PATH" "$TERRAIN_PLANNER_PATH" "$FOLLOWER_PATH" "$REPLAN_MONITOR_PATH" "$INJECTOR_PATH" "$WORLD_DIR/meshes/lunar_terrain.obj" \
...
# ------------------------------------------------------------
# [2/20] Check ROS 2 / Gazebo environment
# ------------------------------------------------------------
echo "[2/20] Checking ROS 2 / Gazebo environment........"
...
log "[2/20] environment OK ($IGN/$MSGNS, $BRIDGE_PKG, slam_toolbox)"

# ------------------------------------------------------------
# [3/20] Clean state
# ------------------------------------------------------------
echo "[3/20] Checking for stale Phase A/I processes......."
...
# only processes belonging to this Phase J command/node contract.
for pattern in "ros_ign_bridge.*parameter_bridge" "ros_gz_bridge.*parameter_bridge" \
               "scripts/control_odometry.py" "scripts/odometry_monitor.py" "scripts/astar_navigation.py" "scripts/dynamic_replan_monitor.py" "slam_toolbox.*online_async"; do
...
sleep 1
echo "      clean state: OK"
log "[3/20] clean state OK"

# ------------------------------------------------------------
# [4/20] Start Gazebo - same validated Phase A world
# ------------------------------------------------------------
echo "[4/20] Starting Gazebo lunar world.................."
...
# sensors, DiffDrive odometry and the Phase J demo can actually advance.
...
log "[4/20] gazebo OK and unpaused (pid $GAZEBO_PID)"

# ------------------------------------------------------------
# [5/20] Spawn LunaBot V4
# ------------------------------------------------------------
echo "[5/20] Spawning LunaBot V4.........................."
...
log "[5/20] spawn OK (z=$SPAWN_Z)"

# ------------------------------------------------------------
# [6/20] ROS 2 <-> Gazebo bridge - same sensor contract
# ------------------------------------------------------------
echo "[6/20] Starting ROS 2 <-> Gazebo bridge............."
...
log "[6/20] bridge OK (pid $BRIDGE_PID)"

# ------------------------------------------------------------
# [7/20] Phase J RGB-D terrain segmentation
# ------------------------------------------------------------
echo "[7/20] Starting terrain segmentation................"
...
log "[7/20] segmentation OK (pid $PERCEPTION_PID)"

# ------------------------------------------------------------
# [8/20] Phase J semantic terrain mapper
# ------------------------------------------------------------
echo "[8/20] Starting semantic terrain mapping..........."
...
log "[8/20] semantic mapper OK (pid $MAPPER_PID)"

# ------------------------------------------------------------
# [9/20] Phase J terrain cost-map generator
# ------------------------------------------------------------
echo "[9/20] Starting terrain cost-map generator........."
...
log "[9/20] cost mapper OK (pid $COST_PID)"

# ------------------------------------------------------------
# [10/20] Phase J terrain-aware path planner
# ------------------------------------------------------------
echo "[10/20] Starting terrain-aware path planner........"
...
log "[10/20] terrain-aware planner OK (pid $TERRAIN_PLANNER_PID)"

# ------------------------------------------------------------
# [11/20] Phase J dynamic replanning monitor
# ------------------------------------------------------------
echo "[11/20] Starting dynamic replanning monitor........"
...
log "[11/20] dynamic replanning monitor OK (pid $REPLAN_MONITOR_PID)"

# ------------------------------------------------------------
# [11b/20] Phase J dynamic obstacle injector (controlled injection)
# ------------------------------------------------------------
echo "[11b/20] Starting dynamic obstacle injector........"
...
echo "      dynamic obstacle injector running (PID $INJECTOR_PID), controlled injection"
log "[11b/20] dynamic obstacle injector OK (pid $INJECTOR_PID)"

# ------------------------------------------------------------
# [12/20] Static TF - unchanged Phase A sensor frames
# ------------------------------------------------------------
echo "[12/20] Starting static TF (sensor frames)............"
TF_SPECS=(
...
log "[12/20] static TF OK"

# ------------------------------------------------------------
# [13/20] Phase J control node
# ------------------------------------------------------------
echo "[13/20] Starting Phase J control layer................"
...
log "[13/20] control OK (pid $CONTROL_PID)"

# ------------------------------------------------------------
# [14/20] Phase J odometry monitor
# ------------------------------------------------------------
echo "[14/20] Starting odometry monitor...................."
...
log "[14/20] odometry monitor OK (pid $ODOM_PID)"

# ------------------------------------------------------------
# [15/20] Phase J SLAM and localization
# ------------------------------------------------------------
echo "[15/20] Starting slam_toolbox mapping................"
...
log "[15/20] slam_toolbox OK (pid $SLAM_PID)"

# ------------------------------------------------------------
# [16/20] Phase J A* autonomous navigation
# ------------------------------------------------------------
echo "[16/20] Starting A* autonomous navigation..........."
...
log "[16/20] diagnostic A* OK (pid $NAV_PID, auto_goal=$AUTO_GOAL, cmd isolated)"

# ------------------------------------------------------------
# [17/20] Phase J terrain-aware motion integration
# ------------------------------------------------------------
echo "[17/20] Starting terrain-aware path follower........"
...
log "[17/20] integration follower OK (pid $FOLLOWER_PID)"

# ------------------------------------------------------------
# [18/20] RViz2
# ------------------------------------------------------------
if [ "$HEADLESS" = "1" ]; then
  echo "[18/20] RViz2 (skipped - HEADLESS).................. OK"
  log "[18/20] rviz skipped (headless)"
else
  echo "[18/20] Starting RViz2.............................."
  setsid rviz2 -d "$RVIZ_CONFIG" -f map >> "$EVIDENCE_DIR/gazebo.log" 2>&1 &
  RVIZ_PID=$!
  sleep 2
  if kill -0 "$RVIZ_PID" 2>/dev/null; then
    echo "      RViz2 running (PID $RVIZ_PID), fixed frame: map"
    log "[18/20] rviz OK (pid $RVIZ_PID)"
  else
    echo "      WARNING: RViz2 exited at startup (continuing without it)."
    log "[18/20] rviz FAILED to start"
    OVERALL="FAIL"
  fi
fi

# ------------------------------------------------------------
# [19/20] Runtime validation
# ------------------------------------------------------------
twist_has_motion() {
...
if [ "$EVIDENCE" = "1" ]; then
  echo ""
  echo "Recording Phase J runtime evidence -> $EVIDENCE_DIR"
...
echo "  evidence/phase-j-launch-j/map_sample.txt"
echo "  evidence/phase-j-launch-j/phase_j_map.yaml/.pgm"
echo "  evidence/phase-j-launch-j/odometry_samples.csv"
echo "  evidence/phase-j-launch-j/odometry_report.txt"
echo "  evidence/phase-j-launch-j/demo_drive_result.txt"
echo "  evidence/phase-j-launch-j/diag_drive.csv"
echo ""
echo "------------------------------------------------------------"
echo "VALIDATION"
echo "------------------------------------------------------------"
...
echo ""
echo "============================================================"
log "[20/20] status printed (overall $OVERALL)"
...
if [ "$DEMO" = "1" ]; then
  ...
  echo ""
  echo "============================================================"
  printf "PHASE J RUN COMPLETE - overall result: %s\n" "$OVERALL"
  echo "============================================================"
  shutdown "$EXIT_CODE"
else
  echo ""
  echo "============================================================"
  echo " Phase J autonomous navigation is running."
  echo ""
  echo " A* auto goal: $AUTO_GOAL"
  echo " RViz can publish a replacement goal on /goal_pose."
  echo " Press Ctrl+C to stop Phase J safely."
  echo "============================================================"
