#!/bin/bash
# ============================================================
#  LunaBot V4 - Phase H  (launch-h)
#  AUTONOMOUS NAVIGATION
# ============================================================
#  Independent launch. It starts the Phase A world and rover directly,
#  then inserts the Phase H ROS control and odometry-monitoring layer.
...
RVIZ_CONFIG="$REPO_DIR/rviz/phase_h.rviz"
EVIDENCE_DIR="$REPO_DIR/evidence/phase-h-launch-h"
...
# that group. This prevents a Phase H relaunch from inheriting old children.
stop_group() {
  local pid="${1:-}"
  [ -n "$pid" ] || return 0
...
save_final_map() {
  ...
  timeout 30 ros2 run nav2_map_server map_saver_cli -f "$EVIDENCE_DIR/phase_h_map" \
    --ros-args -p save_map_timeout:=10.0 2>/dev/null \
    >> "$EVIDENCE_DIR/map_saver.log" 2>&1 || true
  if [ -s "$EVIDENCE_DIR/phase_h_map.yaml" ] && [ -s "$EVIDENCE_DIR/phase_h_map.pgm" ]; then
...
shutdown() {
  ...
  say ""
  say "------------------------------------------------------------"
  say " Shutting down LunaBot Phase H safely..."
  say "------------------------------------------------------------"
  ...
  say "Launch H environment cleanly closed."
  say "Log: $LOG_FILE"
  say "============================================================"
  exit "$rc"
}
...
# ------------------------------------------------------------
# [1/18] Check project files
# ------------------------------------------------------------
echo "[1/18] Checking Phase A baseline + Phase H files....."
missing=0
for f in "$WORLD_PATH" "$MODEL_PATH" "$WASD_PATH" "$CONTROL_PATH" "$ODOM_PATH" "$SLAM_CONFIG" "$NAV_PATH" \
         "$RVIZ_CONFIG" "$PERCEPTION_PATH" "$MAPPER_PATH" "$COST_PATH" "$TERRAIN_PLANNER_PATH" "$WORLD_DIR/meshes/lunar_terrain.obj" \
...
if [ "$missing" -ne 0 ]; then
  echo "      ERROR: required Phase A/H files are missing."
  exit 1
fi
echo "      world, rover, terrain, teleop, controller, monitor, planner, perception, mapper, cost mapper, terrain planner, rviz: OK"
log "[1/18] files OK"

# ------------------------------------------------------------
# [2/18] Check ROS 2 / Gazebo environment
# ------------------------------------------------------------
echo "[2/18] Checking ROS 2 / Gazebo environment........"
...
log "[2/18] environment OK ($IGN/$MSGNS, $BRIDGE_PKG, slam_toolbox)"

# ------------------------------------------------------------
# [3/18] Clean state
# ------------------------------------------------------------
echo "[3/18] Checking for stale Phase A/G processes......."
...
# only processes belonging to this Phase H command/node contract.
...
sleep 1
echo "      clean state: OK"
log "[3/18] clean state OK"

# ------------------------------------------------------------
# [4/18] Start Gazebo - same validated Phase A world
# ------------------------------------------------------------
echo "[4/18] Starting Gazebo lunar world.................."
...
# sensors, DiffDrive odometry and the Phase H demo can actually advance.
...
log "[4/18] gazebo OK and unpaused (pid $GAZEBO_PID)"

# ------------------------------------------------------------
# [5/18] Spawn LunaBot V4
# ------------------------------------------------------------
echo "[5/18] Spawning LunaBot V4.........................."
...
log "[5/18] spawn OK (z=$SPAWN_Z)"

# ------------------------------------------------------------
# [6/18] ROS 2 <-> Gazebo bridge - same sensor contract
# ------------------------------------------------------------
echo "[6/18] Starting ROS 2 <-> Gazebo bridge............."
...
log "[6/18] bridge OK (pid $BRIDGE_PID)"

# ------------------------------------------------------------
# [7/18] Phase H RGB-D terrain segmentation
# ------------------------------------------------------------
echo "[7/18] Starting terrain segmentation................"
...
log "[7/18] segmentation OK (pid $PERCEPTION_PID)"

# ------------------------------------------------------------
# [8/18] Phase H semantic terrain mapper
# ------------------------------------------------------------
echo "[8/18] Starting semantic terrain mapping..........."
...
log "[8/18] semantic mapper OK (pid $MAPPER_PID)"

# ------------------------------------------------------------
# [9/18] Phase H terrain cost-map generator
# ------------------------------------------------------------
echo "[9/18] Starting terrain cost-map generator........."
...
log "[9/18] cost mapper OK (pid $COST_PID)"

# ------------------------------------------------------------
# [10/18] Phase H terrain-aware path planner
# ------------------------------------------------------------
echo "[10/18] Starting terrain-aware path planner........"
...
log "[10/18] terrain-aware planner OK (pid $TERRAIN_PLANNER_PID)"

# ------------------------------------------------------------
# [11/18] Static TF - unchanged Phase A sensor frames
# ------------------------------------------------------------
echo "[11/18] Starting static TF (sensor frames)............"
TF_SPECS=(
...
log "[11/18] static TF OK"

# ------------------------------------------------------------
# [12/18] Phase H control node
# ------------------------------------------------------------
echo "[12/18] Starting Phase H control layer................"
...
log "[12/18] control OK (pid $CONTROL_PID)"

# ------------------------------------------------------------
# [13/18] Phase H odometry monitor
# ------------------------------------------------------------
echo "[13/18] Starting odometry monitor...................."
...
log "[13/18] odometry monitor OK (pid $ODOM_PID)"

# ------------------------------------------------------------
# [14/18] Phase H SLAM and localization
# ------------------------------------------------------------
echo "[14/18] Starting slam_toolbox mapping................"
...
log "[14/18] slam_toolbox OK (pid $SLAM_PID)"

# ------------------------------------------------------------
# [15/18] Phase H A* autonomous navigation
# ------------------------------------------------------------
echo "[15/18] Starting A* autonomous navigation..........."
...
log "[15/18] A* navigation OK (pid $NAV_PID, auto_goal=$AUTO_GOAL)"

# ------------------------------------------------------------
# [16/18] RViz2
# ------------------------------------------------------------
if [ "$HEADLESS" = "1" ]; then
  echo "[16/18] RViz2 (skipped - HEADLESS).................. OK"
  log "[16/18] rviz skipped (headless)"
else
  echo "[16/18] Starting RViz2.............................."
  setsid rviz2 -d "$RVIZ_CONFIG" -f map >> "$EVIDENCE_DIR/gazebo.log" 2>&1 &
  RVIZ_PID=$!
  sleep 2
  if kill -0 "$RVIZ_PID" 2>/dev/null; then
    echo "      RViz2 running (PID $RVIZ_PID), fixed frame: map"
    log "[16/18] rviz OK (pid $RVIZ_PID)"
  else
    echo "      WARNING: RViz2 exited at startup (continuing without it)."
    log "[16/18] rviz FAILED to start"
    OVERALL="FAIL"
  fi
fi

# ------------------------------------------------------------
# [17/18] Runtime validation
# ------------------------------------------------------------
echo "[17/18] Runtime validation.........................."
wait_for_goal() {
...
if [ "$EVIDENCE" = "1" ]; then
  echo ""
  echo "Recording Phase H runtime evidence -> $EVIDENCE_DIR"
...
echo "  evidence/phase-h-launch-h/map_sample.txt"
echo "  evidence/phase-h-launch-h/phase_h_map.yaml/.pgm"
echo "  evidence/phase-h-launch-h/odometry_samples.csv"
echo "  evidence/phase-h-launch-h/odometry_report.txt"
echo "  evidence/phase-h-launch-h/demo_drive_result.txt"
echo "  evidence/phase-h-launch-h/diag_drive.csv"
echo ""
echo "------------------------------------------------------------"
echo "VALIDATION"
echo "------------------------------------------------------------"
...
echo "  - terrain-aware weighted A* publishes a path without motion output"
...
echo ""
echo "============================================================"
log "[18/18] status printed (overall $OVERALL)"
...
if [ "$DEMO" = "1" ]; then
  ...
  echo ""
  echo "============================================================"
  printf "PHASE H RUN COMPLETE - overall result: %s\n" "$OVERALL"
  echo "============================================================"
  shutdown "$EXIT_CODE"
else
  echo ""
  echo "============================================================"
  echo " Phase H autonomous navigation is running."
  echo ""
  echo " A* auto goal: $AUTO_GOAL"
  echo " RViz can publish a replacement goal on /goal_pose."
  echo " Press Ctrl+C to stop Phase H safely."
  echo "============================================================"
