#!/bin/bash
# ============================================================
#  LunaBot V4 - Phase I  (launch-i)
#  AUTONOMOUS NAVIGATION
# ============================================================
#  Independent launch. It starts the Phase A world and rover directly,
#  then inserts the Phase I ROS control and odometry-monitoring layer.
...
RVIZ_CONFIG="$REPO_DIR/rviz/phase_i.rviz"
EVIDENCE_DIR="$REPO_DIR/evidence/phase-i-launch-i"
...
# that group. This prevents a Phase I relaunch from inheriting old children.
stop_group() {
  local pid="${1:-}"
  [ -n "$pid" ] || return 0
...
save_final_map() {
  ...
  if [ "$MAP_SAVER_AVAILABLE" != "1" ]; then
    ...
    log "validation FAIL: Phase I requires nav2_map_server for final map evidence"
    OVERALL="FAIL"
    return 1
  fi
  [ -n "$SLAM_PID" ] || return 0
  ...
  timeout 30 ros2 run nav2_map_server map_saver_cli -f "$EVIDENCE_DIR/phase_i_map" \
    --ros-args -p save_map_timeout:=10.0 2>/dev/null \
    >> "$EVIDENCE_DIR/map_saver.log" 2>&1 || true
  if [ -s "$EVIDENCE_DIR/phase_i_map.yaml" ] && [ -s "$EVIDENCE_DIR/phase_i_map.pgm" ]; then
...
shutdown() {
  ...
  say ""
  say "------------------------------------------------------------"
  say " Shutting down LunaBot Phase I safely..."
  say "------------------------------------------------------------"
  ...
  say "Launch I environment cleanly closed."
  say "Log: $LOG_FILE"
  say "============================================================"
  exit "$rc"
}
...
# ------------------------------------------------------------
# [1/19] Check project files
# ------------------------------------------------------------
echo "[1/19] Checking Phase A baseline + Phase I files....."
missing=0
for f in "$WORLD_PATH" "$MODEL_PATH" "$WASD_PATH" "$CONTROL_PATH" "$ODOM_PATH" "$SLAM_CONFIG" "$NAV_PATH" \
         "$RVIZ_CONFIG" "$PERCEPTION_PATH" "$MAPPER_PATH" "$COST_PATH" "$TERRAIN_PLANNER_PATH" "$FOLLOWER_PATH" "$WORLD_DIR/meshes/lunar_terrain.obj" \
...
# ------------------------------------------------------------
# [2/19] Check ROS 2 / Gazebo environment
# ------------------------------------------------------------
echo "[2/19] Checking ROS 2 / Gazebo environment........"
...
log "[2/19] environment OK ($IGN/$MSGNS, $BRIDGE_PKG, slam_toolbox)"

# ------------------------------------------------------------
# [3/19] Clean state
# ------------------------------------------------------------
echo "[3/19] Checking for stale Phase A/I processes......."
...
# only processes belonging to this Phase I command/node contract.
...
sleep 1
echo "      clean state: OK"
log "[3/19] clean state OK"

# ------------------------------------------------------------
# [4/19] Start Gazebo - same validated Phase A world
# ------------------------------------------------------------
echo "[4/19] Starting Gazebo lunar world.................."
...
# sensors, DiffDrive odometry and the Phase I demo can actually advance.
...
log "[4/19] gazebo OK and unpaused (pid $GAZEBO_PID)"

# ------------------------------------------------------------
# [5/19] Spawn LunaBot V4
# ------------------------------------------------------------
echo "[5/19] Spawning LunaBot V4.........................."
...
log "[5/19] spawn OK (z=$SPAWN_Z)"

# ------------------------------------------------------------
# [6/19] ROS 2 <-> Gazebo bridge - same sensor contract
# ------------------------------------------------------------
echo "[6/19] Starting ROS 2 <-> Gazebo bridge............."
...
log "[6/19] bridge OK (pid $BRIDGE_PID)"

# ------------------------------------------------------------
# [7/19] Phase I RGB-D terrain segmentation
# ------------------------------------------------------------
echo "[7/19] Starting terrain segmentation................"
...
log "[7/19] segmentation OK (pid $PERCEPTION_PID)"

# ------------------------------------------------------------
# [8/19] Phase I semantic terrain mapper
# ------------------------------------------------------------
echo "[8/19] Starting semantic terrain mapping..........."
...
log "[8/19] semantic mapper OK (pid $MAPPER_PID)"

# ------------------------------------------------------------
# [9/19] Phase I terrain cost-map generator
# ------------------------------------------------------------
echo "[9/19] Starting terrain cost-map generator........."
...
log "[9/19] cost mapper OK (pid $COST_PID)"

# ------------------------------------------------------------
# [10/19] Phase I terrain-aware path planner
# ------------------------------------------------------------
echo "[10/19] Starting terrain-aware path planner........"
...
log "[10/19] terrain-aware planner OK (pid $TERRAIN_PLANNER_PID)"

# ------------------------------------------------------------
# [11/19] Static TF - unchanged Phase A sensor frames
# ------------------------------------------------------------
echo "[11/19] Starting static TF (sensor frames)............"
TF_SPECS=(
...
log "[11/19] static TF OK"

# ------------------------------------------------------------
# [12/19] Phase I control node
# ------------------------------------------------------------
echo "[12/19] Starting Phase I control layer................"
...
log "[12/19] control OK (pid $CONTROL_PID)"

# ------------------------------------------------------------
# [13/19] Phase I odometry monitor
# ------------------------------------------------------------
echo "[13/19] Starting odometry monitor...................."
...
log "[13/19] odometry monitor OK (pid $ODOM_PID)"

# ------------------------------------------------------------
# [14/19] Phase I SLAM and localization
# ------------------------------------------------------------
echo "[14/19] Starting slam_toolbox mapping................"
...
log "[14/19] slam_toolbox OK (pid $SLAM_PID)"

# ------------------------------------------------------------
# [15/19] Phase I A* autonomous navigation
# ------------------------------------------------------------
echo "[15/19] Starting A* autonomous navigation..........."
...
log "[15/19] diagnostic A* OK (pid $NAV_PID, auto_goal=$AUTO_GOAL, cmd isolated)"

# ------------------------------------------------------------
# [16/19] Phase I terrain-aware motion integration
# ------------------------------------------------------------
echo "[16/19] Starting terrain-aware path follower........"
...
log "[16/19] integration follower OK (pid $FOLLOWER_PID)"

# ------------------------------------------------------------
# [17/19] RViz2
# ------------------------------------------------------------
if [ "$HEADLESS" = "1" ]; then
  echo "[17/19] RViz2 (skipped - HEADLESS).................. OK"
  log "[17/19] rviz skipped (headless)"
else
  echo "[17/19] Starting RViz2.............................."
  setsid rviz2 -d "$RVIZ_CONFIG" -f map >> "$EVIDENCE_DIR/gazebo.log" 2>&1 &
  RVIZ_PID=$!
  sleep 2
  if kill -0 "$RVIZ_PID" 2>/dev/null; then
    echo "      RViz2 running (PID $RVIZ_PID), fixed frame: map"
    log "[17/19] rviz OK (pid $RVIZ_PID)"
  else
    echo "      WARNING: RViz2 exited at startup (continuing without it)."
    log "[17/19] rviz FAILED to start"
    OVERALL="FAIL"
  fi
fi
...
if [ "$EVIDENCE" = "1" ]; then
  echo ""
  echo "Recording Phase I runtime evidence -> $EVIDENCE_DIR"
...
echo "  evidence/phase-i-launch-i/map_sample.txt"
echo "  evidence/phase-i-launch-i/phase_i_map.yaml/.pgm"
echo "  evidence/phase-i-launch-i/odometry_samples.csv"
echo "  evidence/phase-i-launch-i/odometry_report.txt"
echo "  evidence/phase-i-launch-i/demo_drive_result.txt"
echo "  evidence/phase-i-launch-i/diag_drive.csv"
echo ""
echo "------------------------------------------------------------"
echo "VALIDATION"
echo "------------------------------------------------------------"
...
echo ""
echo "============================================================"
log "[19/19] status printed (overall $OVERALL)"
...
if [ "$DEMO" = "1" ]; then
  ...
  echo ""
  echo "============================================================"
  printf "PHASE I RUN COMPLETE - overall result: %s\n" "$OVERALL"
  echo "============================================================"
  shutdown "$EXIT_CODE"
else
  echo ""
  echo "============================================================"
  echo " Phase I autonomous navigation is running."
  echo ""
  echo " A* auto goal: $AUTO_GOAL"
  echo " RViz can publish a replacement goal on /goal_pose."
  echo " Press Ctrl+C to stop Phase I safely."
  echo "============================================================"
