#!/bin/bash
# ============================================================
#  LunaBot V4 - Phase F  (launch-f)
#  AUTONOMOUS NAVIGATION
# ============================================================
#  Independent launch. It starts the Phase A world and rover directly,
#  then inserts the Phase F ROS control and odometry-monitoring layer.
...
RVIZ_CONFIG="$REPO_DIR/rviz/phase_f.rviz"
EVIDENCE_DIR="$REPO_DIR/evidence/phase-f-launch-f"
...
# that group. This prevents a Phase F relaunch from inheriting old children.
stop_group() {
  local pid="${1:-}"
  [ -n "$pid" ] || return 0
...
save_final_map() {
  ...
  timeout 30 ros2 run nav2_map_server map_saver_cli -f "$EVIDENCE_DIR/phase_f_map" \
    --ros-args -p save_map_timeout:=10.0 2>/dev/null \
    >> "$EVIDENCE_DIR/map_saver.log" 2>&1 || true
  if [ -s "$EVIDENCE_DIR/phase_f_map.yaml" ] && [ -s "$EVIDENCE_DIR/phase_f_map.pgm" ]; then
...
shutdown() {
  ...
  say ""
  say "------------------------------------------------------------"
  say " Shutting down LunaBot Phase F safely..."
  say "------------------------------------------------------------"
  ...
  say "Launch F environment cleanly closed."
  say "Log: $LOG_FILE"
  say "============================================================"
  exit "$rc"
}
...
# ------------------------------------------------------------
# [1/16] Check project files
# ------------------------------------------------------------
echo "[1/16] Checking Phase A baseline + Phase F files....."
missing=0
for f in "$WORLD_PATH" "$MODEL_PATH" "$WASD_PATH" "$CONTROL_PATH" "$ODOM_PATH" "$SLAM_CONFIG" "$NAV_PATH" \
         "$RVIZ_CONFIG" "$PERCEPTION_PATH" "$MAPPER_PATH" "$WORLD_DIR/meshes/lunar_terrain.obj" \
...
if [ "$missing" -ne 0 ]; then
  echo "      ERROR: required Phase A/F files are missing."
  exit 1
fi
echo "      world, rover, terrain, teleop, controller, monitor, planner, perception, mapper, rviz: OK"
log "[1/16] files OK"

# ------------------------------------------------------------
# [2/16] Check ROS 2 / Gazebo environment
# ------------------------------------------------------------
echo "[2/16] Checking ROS 2 / Gazebo environment........"
...
log "[2/16] environment OK ($IGN/$MSGNS, $BRIDGE_PKG, slam_toolbox)"

# ------------------------------------------------------------
# [3/16] Clean state
# ------------------------------------------------------------
echo "[3/16] Checking for stale Phase A/F processes......."
...
# only processes belonging to this Phase F command/node contract.
...
sleep 1
echo "      clean state: OK"
log "[3/16] clean state OK"

# ------------------------------------------------------------
# [4/16] Start Gazebo - same validated Phase A world
# ------------------------------------------------------------
echo "[4/16] Starting Gazebo lunar world.................."
...
# sensors, DiffDrive odometry and the Phase F demo can actually advance.
...
log "[4/16] gazebo OK and unpaused (pid $GAZEBO_PID)"

# ------------------------------------------------------------
# [5/16] Spawn LunaBot V4
# ------------------------------------------------------------
echo "[5/16] Spawning LunaBot V4.........................."
...
log "[5/16] spawn OK (z=$SPAWN_Z)"

# ------------------------------------------------------------
# [6/16] ROS 2 <-> Gazebo bridge - same sensor contract
# ------------------------------------------------------------
echo "[6/16] Starting ROS 2 <-> Gazebo bridge............."
...
log "[6/16] bridge OK (pid $BRIDGE_PID)"

# ------------------------------------------------------------
# [7/16] Phase F RGB-D terrain segmentation
# ------------------------------------------------------------
echo "[7/16] Starting terrain segmentation................"
...
log "[7/16] segmentation OK (pid $PERCEPTION_PID)"

# ------------------------------------------------------------
# [8/16] Phase F semantic terrain mapper
# ------------------------------------------------------------
echo "[8/16] Starting semantic terrain mapping..........."
...
log "[8/16] semantic mapper OK (pid $MAPPER_PID)"

# ------------------------------------------------------------
# [9/16] Static TF - unchanged Phase A sensor frames
# ------------------------------------------------------------
echo "[9/16] Starting static TF (sensor frames)............"
TF_SPECS=(
...
log "[9/16] static TF OK"

# ------------------------------------------------------------
# [10/16] Phase F control node
# ------------------------------------------------------------
echo "[10/16] Starting Phase F control layer................"
...
log "[10/16] control OK (pid $CONTROL_PID)"

# ------------------------------------------------------------
# [11/16] Phase F odometry monitor
# ------------------------------------------------------------
echo "[11/16] Starting odometry monitor...................."
...
log "[11/16] odometry monitor OK (pid $ODOM_PID)"

# ------------------------------------------------------------
# [12/16] Phase F SLAM and localization
# ------------------------------------------------------------
echo "[12/16] Starting slam_toolbox mapping................"
...
log "[12/16] slam_toolbox OK (pid $SLAM_PID)"

# ------------------------------------------------------------
# [13/16] Phase F A* autonomous navigation
# ------------------------------------------------------------
echo "[13/16] Starting A* autonomous navigation..........."
...
log "[13/16] A* navigation OK (pid $NAV_PID, auto_goal=$AUTO_GOAL)"

# ------------------------------------------------------------
# [14/16] RViz2
# ------------------------------------------------------------
if [ "$HEADLESS" = "1" ]; then
  echo "[14/16] RViz2 (skipped - HEADLESS).................. OK"
  log "[14/16] rviz skipped (headless)"
else
  echo "[14/16] Starting RViz2.............................."
  setsid rviz2 -d "$RVIZ_CONFIG" -f map >> "$EVIDENCE_DIR/gazebo.log" 2>&1 &
  RVIZ_PID=$!
  sleep 2
  if kill -0 "$RVIZ_PID" 2>/dev/null; then
    echo "      RViz2 running (PID $RVIZ_PID), fixed frame: map"
    log "[14/16] rviz OK (pid $RVIZ_PID)"
  else
    echo "      WARNING: RViz2 exited at startup (continuing without it)."
    log "[14/16] rviz FAILED to start"
    OVERALL="FAIL"
  fi
fi

# ------------------------------------------------------------
# [15/16] Runtime validation
# ------------------------------------------------------------
echo "[15/16] Runtime validation.........................."
wait_for_goal() {
...
topic_ok() {
  ...
  case "$2" in
    /map|/plan|/lunabot/navigation/status|/lunabot/terrain/semantic_map|/lunabot/terrain/semantic_map/status) durability="transient_local" ;;
  esac
...
if [ "$EVIDENCE" = "1" ]; then
  echo ""
  echo "Recording Phase F runtime evidence -> $EVIDENCE_DIR"
...
echo "  evidence/phase-f-launch-f/map_sample.txt"
echo "  evidence/phase-f-launch-f/phase_f_map.yaml/.pgm"
echo "  evidence/phase-f-launch-f/odometry_samples.csv"
echo "  evidence/phase-f-launch-f/odometry_report.txt"
echo "  evidence/phase-f-launch-f/demo_drive_result.txt"
echo "  evidence/phase-f-launch-f/diag_drive.csv"
echo ""
echo "------------------------------------------------------------"
echo "VALIDATION"
echo "------------------------------------------------------------"
...
echo ""
echo "============================================================"
log "[16/16] status printed (overall $OVERALL)"
...
if [ "$DEMO" = "1" ]; then
  ...
  echo ""
  echo "============================================================"
  printf "PHASE F RUN COMPLETE - overall result: %s\n" "$OVERALL"
  echo "============================================================"
  shutdown "$EXIT_CODE"
else
  echo ""
  echo "============================================================"
  echo " Phase F autonomous navigation is running."
  echo ""
  echo " A* auto goal: $AUTO_GOAL"
  echo " RViz can publish a replacement goal on /goal_pose."
  echo " Press Ctrl+C to stop Phase F safely."
  echo "============================================================"
